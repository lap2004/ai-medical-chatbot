...
# from sqlalchemy import Column, DateTime, ForeignKey, Text, func, Enum, UniqueConstraint
# from sqlalchemy.dialects.postgresql import UUID
# from db.database import Base
# from db.models.enums import FeedbackValue, ReportStatus

# class MessageFeedback012026(Base):
#     __tablename__ = "message_feedback_012026"
#     __table_args__ = (
#         UniqueConstraint("message_id", "user_id", name="uq_message_feedback_012026"),
#     )

#     id = Column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
#     message_id = Column(ForeignKey("messages_012026.id", ondelete="CASCADE"), nullable=False, index=True)
#     user_id = Column(ForeignKey("users_012026.id", ondelete="CASCADE"), nullable=False)

#     value = Column(Enum(FeedbackValue, name="feedback_value"), nullable=False)
#     reason = Column(Text)

#     created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
#     updated_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())

# class MessageReport012026(Base):
#     __tablename__ = "message_reports_012026"
#     __table_args__ = (
#         UniqueConstraint("message_id", "reporter_user_id", name="uq_message_report_012026"),
#     )

#     id = Column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
#     message_id = Column(ForeignKey("messages_012026.id", ondelete="CASCADE"), nullable=False)
#     reporter_user_id = Column(ForeignKey("users_012026.id", ondelete="CASCADE"), nullable=False)

#     category = Column(Text, nullable=False)
#     details = Column(Text)
#     status = Column(Enum(ReportStatus, name="report_status"), nullable=False, server_default="open")
