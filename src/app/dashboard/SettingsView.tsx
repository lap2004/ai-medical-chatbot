...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = (props) => (
    <input
        {...props}
        className={`w-full h-10 px-3 rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-teal-100 text-[13px] font-semibold bg-white ${props.className ?? ""}`}
    />
);
...
export default function SettingsView() {
    ...
    const handleSysChange = async <K extends keyof SystemSettings>(key: K, value: SystemSettings[K]) => {
        ...
        try {
            ...
            setSysMsg({ text: "Settings saved.", type: "success" });
        } catch (e: any) {
            setSysSettings(sysSettings); // revert
            setSysMsg({ text: e?.response?.data?.detail ?? "Failed to save.", type: "error" });
        } finally {
            setSysLoading(false);
    ...
    const saveProfile = async () => {
        ...
        try {
            ...
            setProfileMsg({ text: e?.response?.data?.detail ?? "Failed to update profile.", type: "error" });
        } finally {
    ...
    const savePassword = async () => {
        ...
        if (pwForm.new_password !== pwForm.confirm) {
            setProfileMsg({ text: "Mật khẩu mới không khớp.", type: "error" });
            return;
        }
        if (pwForm.new_password.length < 6) {
            setProfileMsg({ text: "Mật khẩu mới phải ít nhất 6 ký tự.", type: "error" });
            return;
        }
        setProfileLoading(true);
        setProfileMsg(null);
        try {
            ...
            setProfileMsg({ text: "Password changed.", type: "success" });
        } catch (e: any) {
            setProfileMsg({ text: e?.response?.data?.detail ?? "Failed to change password.", type: "error" });
        } finally {
            setProfileLoading(false);
            setTimeout(() => setProfileMsg(null), 3500);
        }
    ...
    return (
        <div className="max-w-2xl space-y-6">
            <div className="text-[11px] text-slate-400 bg-amber-50 border border-amber-200 px-4 py-2 rounded-xl">
                ⚠️ Các cấu hình AI/System sẽ reset về mặc định nếu server restart.
            </div>
                ...
                <div className="flex items-center justify-between pt-1">
                    ...
                    <div className="ml-auto">
                        <button
                            ...
                            className="h-9 px-5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-[12px] font-extrabold disabled:opacity-50"
                        >
                            Change Password
                        </button>
                    </div>
            ...
            <SectionCard
                ...
                title="System & AI Config"
                subtitle="Runtime configuration — resets on server restart"
            >
                {sysSettings ? (
                    <>
                        {/* Allow Signup */}
                        <div className="flex items-center justify-between py-2 border-b border-slate-100">
                            <div>
                                <div className="text-[13px] font-bold text-slate-800">User Self-Registration</div>
                                <div className="text-[11px] text-slate-400">
                                    Allow new users to register from the signup page
                                </div>
                            </div>
                            <Toggle
                        ...
                        <div className="flex items-center justify-between py-2 border-b border-slate-100">
                            <div>
                                <div className="text-[13px] font-bold text-slate-800">RAG Top-K</div>
                                <div className="text-[11px] text-slate-400">
                            ...
                            <input
                                ...
                                className="w-20 h-9 text-center rounded-xl border border-slate-200 outline-none focus:ring-2 focus:ring-teal-100 text-[13px] font-bold"
                            />
                        </div>
                        ...
                        <div className="flex items-center justify-between py-2">
                            <div>
                                <div className="text-[13px] font-bold text-slate-800">Gemini Model</div>
                                <div className="text-[11px] text-slate-400">
                                    Active Gemini model used for all chat responses
                                </div>
                            </div>
                            <select
                                ...
                                disabled={sysLoading}
                                className="h-9 px-3 rounded-xl border border-slate-200 text-[12px] font-semibold outline-none focus:ring-2 focus:ring-teal-100"
                            >
